...
const Education = () => {
    ...
    return (
        <ErrorBoundary fallback={<p>Something went wrong</p>}>
            <div id="Education" className="w-full h-full p-4 rounded-xl shadow sm:p-6 border-gray-200 dark:border-gray-700 bg-black">
                ...
                <ul className="my-4">
                    {EducationList.map(({ college, degree, location, link, fromDate, toDate }, index) => (
                        <a key={index} href={link} target="_blank">
                            <li className={`cursor-pointer w-full h-full rounded overflow-hidden bg-zinc hover:bg-gray-800 px-4 py-2 scale_on_hover ${(index == EducationList.length - 1) ? "mb-1" : "mb-5"}`}>
